...
import { AlertTriangle, ArrowLeft, Brain, CheckCircle2, Clapperboard, DoorOpen, ExternalLink, EyeOff, Gamepad2, Link, Lock, MapPin, Maximize2, Minimize2, MousePointerClick, Pencil, Play, Plus, RefreshCw, RotateCcw, Trash2, XCircle } from 'lucide-react';
import { applyProjectStartType } from '../../../shared/services/cinematicEngine';
import { getSceneActionSources } from '../../../shared/services/projectTransitions';
...
const formatRouteSyncFeedback = ({ addedRooms = 0, addedConnections = 0, lockedConnections = 0 } = {}) => (
  `${addedRooms} pièce${addedRooms > 1 ? 's' : ''} ajoutée${addedRooms > 1 ? 's' : ''}, `
  + `${addedConnections} liaison${addedConnections > 1 ? 's' : ''} ajoutée${addedConnections > 1 ? 's' : ''}, `
  + `${lockedConnections} verrouillée${lockedConnections > 1 ? 's' : ''}.`
);
...
export default function SceneRouteMap({ project, patchProject, getSceneLabel, setSelectedSceneId, setTab }) {
  ...
  const synchronizeFromProjectLogic = () => {
    const result = synchronizeRouteMapFromProject({
      project,
      routeMap,
      actId: activeActId,
      getSceneLabel,
    ...
    patchRouteMap((draftMap) => {
      draftMap.rows = result.routeMap.rows;
      draftMap.cols = result.routeMap.cols;
      draftMap.cells = result.routeMap.cells;
      draftMap.rooms = result.routeMap.rooms;
      draftMap.connections = result.routeMap.connections;
      draftMap.canvases = result.routeMap.canvases;
      draftMap.notes = result.routeMap.notes;
      if (result.routeMap.gameplayState) {
        draftMap.gameplayState = result.routeMap.gameplayState;
      } else {
        delete draftMap.gameplayState;
      }
    });
    if (result.firstAddedCanvasId) setActiveCanvasId(result.firstAddedCanvasId);
    if (result.firstAddedRoomId) setSelectedRoomId(result.firstAddedRoomId);
    setSelectedConnectionId('');
    setConnectFromId('');
    setSyncFeedback(formatRouteSyncFeedback(result));
  };
  ...
  if (routeSection === 'home') {
    return (
      <section className="panel route-map-choice-shell" data-tour="map-home">
        ...
        <div className="route-choice-grid">
          <button type="button" className="route-choice-card build" onClick={openBuilderView} data-tour="map-open-builder">
          ...
          <button type="button" className="route-choice-card tests" onClick={openTestsView} data-tour="map-open-tests">
            <span className="route-choice-image">
        ...
        <div className="route-choice-summary" aria-label="Résumé du plan" data-tour="map-summary">
    ...
    <div className="layout route-map-layout">
      <section className="panel side route-map-tools">
        <div className="panel-head route-view-panel-head">
          <div>
            <button type="button" className="secondary-action route-back-button" onClick={returnToRouteHome} data-tour="map-back-home">
        ...
        <label className="route-act-picker" data-tour="map-act-picker">
          Acte
        ...
        <div className={`route-view-status ${isGameplayMode ? 'tests' : 'build'}`} role="status" data-tour="map-view-status">
        ...
        {isGameplayMode ? (
          <div className={`route-gameplay-card ${gameplay.reachedEnd ? 'gameplay-complete' : ''}`} data-tour="map-gameplay-card">
        ...
        {!isGameplayMode ? <div className="inline-actions" data-tour="map-add-room">
          <button type="button" disabled={isActiveCanvasFull} onClick={() => addRoom()} data-tour="map-add-room-manual">
            <Plus size={16} aria-hidden="true" />
            Pièce
          </button>
          <button type="button" className="secondary-action" onClick={addMissingSceneRooms} data-tour="map-add-scenes">
            <DoorOpen size={16} aria-hidden="true" />
            Depuis scènes
          </button>
          <button type="button" className="secondary-action route-auto-sync-button" onClick={synchronizeFromProjectLogic} data-tour="map-sync-logic">
            <RefreshCw size={16} aria-hidden="true" />
            Synchroniser depuis la logique
          </button>
          {syncFeedback ? (
            <span className="route-sync-feedback" role="status" aria-live="polite">
              {syncFeedback}
            </span>
          ) : null}
        </div> : null}
        ...
        {!isGameplayMode ? (
          <div className="route-canvas-picker" data-tour="map-canvas-picker">
            <label>
              Canvas actif
            ...
            <button type="button" className="secondary-action" onClick={addCanvas} data-tour="map-new-canvas">
              <Plus size={16} aria-hidden="true" />
              Nouveau canvas
            </button>
            {isActiveCanvasFull ? (
        ...
        {!isGameplayMode ? (
          <>
            <label className="route-notes-field" data-tour="map-notes">
              Notes de parcours
              <textarea
      ...
      <section className={`panel main route-map-main ${isFullscreen ? 'fullscreen' : ''}`} data-tour="map-main">
      ...
      <section className="panel side route-map-inspector">
        ...
        {isGameplayMode ? (
          <div className="editor-stack route-gameplay-inspector" data-tour="map-gameplay-detail">
