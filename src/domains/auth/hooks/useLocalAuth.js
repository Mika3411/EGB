...
import {
  ...
  subscribeToRemoteAuthStateChanges,
} from '../../../shared/services/remoteSession';
import { migrateProjectAssetReferences } from '../../../shared/services/assetManager';
import { canUseLocalStorage, readJsonStorage, removeStorageKey } from '../../../shared/utils/storageHelpers';
import { deleteProjectLocalDrafts } from '../../../shared/utils/projectDraftCleanup';
...
const persistSingleProject = async (userId, project, projects, options = {}) => {
  ...
  const remoteAttempted = !options.localOnly && hasRemoteStorageConfig();
...
const persistProjects = async (userId, projects, options = {}) => {
  ...
  const localCacheSaved = localWriteStatus.cacheSaved;
  const localPartial = !localSaved && localCacheSaved;
  const remoteAttempted = hasRemoteStorageConfig();
  let remoteSaved = false;
  let remoteError = '';
  try {
  ...
  useEffect(() => {
    ...
    async function hydrateUserProjects() {
      ...
      try {
        ...
        // pour qu'il apparaisse dans la ProfileWorkspace au lieu d'afficher 0 projet.
        if (localProjects.length === 0) {
