...
function reduceCombinationAction(state, action) {
  ...
  return withResult({
    ...
    viewerImage: createInventoryViewerImage(state.project, resultItem) || null,
...
function applyCinematicEndToState(state, cinematic) {
  ...
  if (endEvent.type === 'item' && endEvent.itemId) {
    ...
    return {
      ...
      viewerImage: createInventoryViewerImage(state.project, rewardItem) || null,
    };
  }

  return { ...state, viewerImage: null };
}
...
function reduceGameAction(state, action) {
  ...
  if (['inventory/add', 'inventory_add', 'add_item'].includes(type)) {
    return withResult({
      ...
      viewerImage: createInventoryViewerImage(state.project, action.itemId) || null,
